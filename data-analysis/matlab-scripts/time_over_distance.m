...
for file = 1:numel(filenamesGuessO)
    ...
    if id == prevID
        h(end+1) = plot(dist, time, 'r', 'DisplayName', 'Guess O Meter 2nd attempt');
    else
        h(end+1) = plot(dist, time, 'r--', 'DisplayName', 'Guess O Meter 1st attempt');
    end
...
for file = 1:numel(filenamesDifferentiated)
    ...
    if id == prevID
        h(end+1) = plot(dist, time, 'b', 'DisplayName', 'Diff+COPE1 2nd attempt');
    else
        h(end+1) = plot(dist, time, 'b--', 'DisplayName', 'Diff+COPE1 1st attempt');
    end
...
ylabel("Time [s]")
xlabel("Dist [m]")
title("State of speed over distance traveled")
